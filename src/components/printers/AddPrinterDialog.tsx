...
import { usePrinters, Supply } from '@/context/PrinterContext';
...
export function AddPrinterDialog() {
  ...
  const [form, setForm] = useState({
    ip: '', hostname: '', brand: '', model: '', serial: '',
    firmware: '', mac: '', location: '', sector: '', isColor: false,
  });
  ...
  const handleLoadInfo = async () => {
    if (!form.ip) {
      toast({ title: 'IP obrigatório', description: 'Informe o IP da impressora.', variant: 'destructive' });
      return;
    }
    setLoading(true);
    try {
      ...
      toast({ title: `${found.length} informações encontradas`, description: found.length > 0 ? `Carregado: ${found.join(', ')}` : 'Nenhuma informação reconhecida.' });
    } catch {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast({ title: 'Impressora adicionada', description: `${form.brand} ${form.model} (${form.ip}) salva com sucesso.` });
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
        <DialogHeader><DialogTitle>Adicionar Impressora</DialogTitle></DialogHeader>
        <div className="space-y-4 pt-2">
          ...
          <Button className="w-full" onClick={handleSubmit} disabled={submitting}>
            ...
            {submitting ? 'Salvando...' : 'Adicionar Impressora'}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
